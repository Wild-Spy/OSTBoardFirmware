...
//    rx_sum1 += byte;
//    rx_sum2 += rx_sum1;
    //need to do this to line up with the python version!
...
//        PORT->Group[0].OUTSET.reg = (1 << 27);
    ...
    switch(rx_frame_state) {
        ...
        case RECEIVING_CHECKSUM_LOW:
            ...
            uint16_t final_checksum = fletcher16_rx_finalize();
            if(rx_frame_checksum != final_checksum) {
                /* Frame fails the checksum and so is dropped */
//                report_printf("rx - drop bad chksum %u != %u", rx_frame_checksum, final_checksum);
                rx_frame_state = SEARCHING_FOR_SOF;
            }
            else {
 ...
 *
 * control encodes the length_ of the frame in the bottom four bits. The
...
uint8_t min_frame_length(uint8_t control_byte) {
    return control_byte&FRAME_LENGTH_MASK;
}